...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.P("📊 Geographical Data Analysis Platform", 
               style={
                   'textAlign': 'center',
                   'fontSize': '16px',
                   'color': '#7f8c8d',
    ...
    # 標籤頁導航
    html.Div([
        dcc.Tabs(id="main-tabs", value='map', children=[
            dcc.Tab(label='📋 Query', value='query', style=tab_style, selected_style=tab_selected_style),
            ...
            dcc.Tab(label='🌡️ ComplexHeatmap', value='complex-heatmap', style=tab_style, selected_style=tab_selected_style),
            dcc.Tab(label='🗺️ Map', value='map', style=tab_style, selected_style=tab_selected_style),
            dcc.Tab(label='🧬 cgMLST', value='cgmlst', style=tab_style, selected_style=tab_selected_style),
...
def render_content(tab):
    if tab == 'query':
        return html.Div([
            html.H3("📋 Query Analysis", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            html.Div([
                html.Div([
                    html.H4("🔢 數據查詢工具", style={'color': '#34495e'}),
                    html.P("這裡可以進行複雜的數據查詢和篩選操作。", style={'color': '#7f8c8d'}),
                    html.Div([
                        html.Label("選擇查詢類型:", style={'fontWeight': 'bold', 'marginBottom': '10px'}),
                        dcc.Dropdown(
                            options=[
                                {'label': 'SQL Query', 'value': 'sql'},
                                {'label': 'NoSQL Query', 'value': 'nosql'},
                        ...
                        html.Textarea(
                            ...
                            style={'width': '100%', 'height': '120px', 'padding': '10px', 'fontSize': '14px'}
                        )
        ...
        return html.Div([
            html.H3("🔍 Advanced Search", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            html.Div([
                html.H4("🎯 智能搜索引擎", style={'color': '#34495e'}),
                html.P("使用先進的搜索算法快速找到相關數據。", style={'color': '#7f8c8d'}),
                html.Div([
                    dcc.Input(
                        placeholder="輸入搜索關鍵詞...",
                    ...
                    html.Button(
                        "🔍 搜索", 
                        style={'padding': '12px 24px', 'backgroundColor': '#3498db', 'color': 'white', 'border': 'none', 'borderRadius': '5px', 'cursor': 'pointer'}
                    )
                ], style={'marginBottom': '20px'}),
                html.Div([
                    html.P("搜索結果將顯示在這裡...", style={'color': '#95a5a6', 'fontStyle': 'italic'})
                ], style={'backgroundColor': '#f8f9fa', 'padding': '20px', 'borderRadius': '10px', 'minHeight': '200px'})
            ])
        ], style={'padding': '20px'})
        
    elif tab == 'heatmap':
        return html.Div([
            html.H3("🔥 Heatmap Visualization", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            html.Div([
                html.H4("🌡️ 熱力圖分析", style={'color': '#34495e'}),
                html.P("生成數據的熱力圖視覺化，識別模式和趨勢。", style={'color': '#7f8c8d'}),
                html.Div([
                    html.P("熱力圖將在這裡顯示...", style={'textAlign': 'center', 'color': '#95a5a6', 'fontSize': '18px', 'marginTop': '80px'})
                ], style={'backgroundColor': '#f8f9fa', 'padding': '40px', 'borderRadius': '10px', 'minHeight': '400px', 'border': '2px dashed #bdc3c7'})
            ])
        ], style={'padding': '20px'})
        
    elif tab == 'complex-heatmap':
        return html.Div([
            html.H3("🌡️ Complex Heatmap Analysis", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            html.Div([
                html.H4("🔬 複雜熱力圖", style={'color': '#34495e'}),
                html.P("多維度數據的複雜熱力圖分析，支持聚類和註釋。", style={'color': '#7f8c8d'}),
                html.Div([
                    html.P("複雜熱力圖將在這裡顯示...", style={'textAlign': 'center', 'color': '#95a5a6', 'fontSize': '18px', 'marginTop': '80px'})
                ], style={'backgroundColor': '#f8f9fa', 'padding': '40px', 'borderRadius': '10px', 'minHeight': '400px', 'border': '2px dashed #bdc3c7'})
            ])
        ], style={'padding': '20px'})
        ...
        return html.Div([
            html.H3("🗺️ Geographical Distribution", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            ...
            html.Div([
                html.Div([
                    ...
                    html.Div(id='map-status-tabs', style={'color': '#27ae60', 'fontSize': '12px', 'fontStyle': 'italic'})
        ...
        return html.Div([
            html.H3("🧬 cgMLST Analysis", style={'color': '#2c3e50', 'marginBottom': '20px'}),
            html.Div([
                html.H4("🔬 核心基因組多位點序列分型", style={'color': '#34495e'}),
                html.P("Core Genome Multi-Locus Sequence Typing 分析工具。", style={'color': '#7f8c8d'}),
                html.Div([
                    html.P("cgMLST 分析結果將在這裡顯示...", style={'textAlign': 'center', 'color': '#95a5a6', 'fontSize': '18px', 'marginTop': '80px'})
                ], style={'backgroundColor': '#f8f9fa', 'padding': '40px', 'borderRadius': '10px', 'minHeight': '400px', 'border': '2px dashed #bdc3c7'})
            ])
        ], style={'padding': '20px'})
