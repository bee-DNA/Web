...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.P("✅ 已確認可正常切換的地圖樣式：一般地圖 | 衛星圖 | 地形圖", 
               style={
                   'textAlign': 'center',
                   'fontSize': '16px',
                   'color': '#27ae60',
    ...
    html.Div([
        html.Div([
            html.Label("🗺️ 選擇地圖樣式:", style={'fontWeight': 'bold', 'marginBottom': '10px', 'color': '#2c3e50', 'fontSize': '16px'}),
            dcc.RadioItems(
                id='map-style-radio',
                options=[
                    {'label': ' 🗺️ 標準街道地圖 (OpenStreetMap)', 'value': 'open-street-map'},
                    {'label': ' 🌫️ 淺色簡約地圖 (Carto Light)', 'value': 'carto-positron'},
                    {'label': ' 🌙 深色主題地圖 (Carto Dark)', 'value': 'carto-darkmatter'},
                    {'label': ' 🏔️ 地形高度地圖 (Stamen Terrain)', 'value': 'stamen-terrain'},
                    {'label': ' 🎨 水彩藝術地圖 (Stamen Watercolor)', 'value': 'stamen-watercolor'},
                    {'label': ' 🌍 基礎地圖 (Basic)', 'value': 'basic'},
                    {'label': ' 🛰️ 衛星街道地圖 (Satellite)', 'value': 'satellite-streets'}
                ],
                value='open-street-map',
                style={'marginBottom': '20px', 'fontSize': '14px'},
                labelStyle={'display': 'block', 'marginBottom': '6px', 'cursor': 'pointer', 'padding': '4px'}
            ),
            html.Div(id='map-status', style={'color': '#27ae60', 'fontSize': '12px', 'fontStyle': 'italic'})
...
def show_current_map_info(map_style, range_filter):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.H5(map_info['name'], style={'margin': '0', 'color': map_info['color']}),
                html.P(map_info['desc'], style={'margin': '5px 0', 'color': '#7f8c8d', 'fontSize': '14px'}),
                html.P(f"篩選範圍: {range_filter}", style={'margin': '0', 'fontSize': '12px', 'color': '#95a5a6'})
            ], style={
                'padding': '15px',
