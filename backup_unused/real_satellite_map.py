...
@app.callback(
    ...
    [Input("real-map-style", "value"), Input("real-mapbox-token", "value")],
)
...
# 統計資訊回調
@app.callback(
    Output("real-stats-info", "children"), [Input("real-range-filter", "value")]
)
                ...
                countries_with_data.append(
                    {
                        ...
                        "lon": info["lon"],
                        "city_count": len(
                            [city for city in info["cities"] if city in cities_in_data]
                        ),
                    }
                )
        ...
        if countries_with_data:
            ...
            fig.add_trace(
                go.Scattermapbox(
                    ...
                    lon=country_lons,
                    mode="markers",
                    marker=dict(size=12, color="#e67e22", opacity=0.8, symbol="circle"),
                    name="📍 有數據的國家",
                    hovertemplate="<b>🏛️ %{customdata[0]}</b><br>"
